...
#else
static void dx_aead_exit(struct crypto_aead *tfm)
#endif
{
...
//	DX_LOG_DEBUG("Clearing context @%p for %s\n",
//		crypto_aead_ctx(tfm), crypto_tfm_alg_name(tfm));
#endif
...
#else
static int dx_aead_init(struct crypto_aead *tfm)
#endif
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,3,0)
	struct dx_aead_ctx *ctx = crypto_tfm_ctx(tfm);
	...
	struct dx_aead_ctx *ctx = crypto_aead_ctx(tfm);
	struct dx_crypto_alg *dx_alg =
			container_of(alg, struct dx_crypto_alg, aead_alg);
#endif
	struct device *dev;
	...
	if (areq_ctx->gen_ctx.op_type == SEP_CRYPTO_DIRECTION_DECRYPT) {
		...
		if (unlikely(areq_ctx->is_icv_fragmented == true))
			...
			dx_buffer_mgr_copy_scatterlist_portion(
				...
				areq->cryptlen+areq_ctx->dstOffset + ctx->authsize, DX_SG_FROM_BUF);
...
static int dx_aead_process(struct aead_request *req, enum sep_crypto_direction direct)
{
	...
	if (unlikely(validate_data_size(ctx, direct, req) != 0)) {
		DX_LOG_ERR("Unsupported crypt/assoc len %u/%u.\n",
...
static struct dx_crypto_alg *dx_aead_create_alg(struct dx_alg_template *template)
{
	...
	alg->base.cra_priority = DX_CRA_PRIO;

	alg->base.cra_ctxsize = sizeof(struct dx_aead_ctx);
	alg->base.cra_flags = CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY |
			 template->type;
    alg->init = dx_aead_init;
	alg->exit = dx_aead_exit;
