	...
	if (unlikely(req->src == req->dst)) {
		...
		if (unlikely(areq_ctx->dma_buf_type == DX_DMA_BUF_MLLI)) {
			...
			dx_buffer_mgr_add_scatterlist_entry(&sg_data,
				...
				req->nbytes,0, true, &areq_ctx->in_mlli_nents);
		}
	} else {
	...
	struct device *dev, struct aead_request *req)
{
...
#endif
	DX_LOG_DEBUG("Unmapping src sgl: req->src=%pK areq_ctx->src.nents=%u areq_ctx->assoc.nents=%u assoclen:%u cryptlen=%u\n", sg_virt(req->src),areq_ctx->src.nents,areq_ctx->assoc.nents,req->assoclen,req->cryptlen);
	DX_RESTORE_DMA_ADDR_TO_48BIT(sg_dma_address(req->src));
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,3,0)
	...
	dma_unmap_sg(dev, req->src, dx_buffer_mgr_get_sgl_nents(req->src,size_to_unmap,&dummy,&chained) , DMA_BIDIRECTIONAL);
	if (unlikely(req->src != req->dst)) {
		DX_LOG_DEBUG("Unmapping dst sgl: req->dst=%pK\n", 
			sg_virt(req->dst));
		DX_RESTORE_DMA_ADDR_TO_48BIT(sg_dma_address(req->dst));
		dma_unmap_sg(dev, req->dst, dx_buffer_mgr_get_sgl_nents(req->dst,size_to_unmap,&dummy,&chained),
			DMA_BIDIRECTIONAL);
	}
#endif
...
#else
    {
    	...
    	dx_buffer_mgr_copy_scatterlist_portion(
			...
			size_to_skip+ req->cryptlen, DX_SG_FROM_BUF);
    }
#endif
	...
	bool is_last, bool do_chain)
{
	...
	DX_LOG_DEBUG("Mapped iv %u B at va=%pK to dma=0x%llX do_chain=%u\n",
		hw_iv_size, req->iv, 
		(unsigned long long)areq_ctx->gen_ctx.iv_dma_addr,do_chain);
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,3,0)
	if (do_chain == true) {
		struct crypto_aead *tfm = crypto_aead_reqtfm(req);
	...
	bool is_last_table)
{
	...
	if (likely(req->src == req->dst)) {
		...
		icv_nents = dx_buffer_mgr_get_aead_icv_nents(areq_ctx->srcSgl,
		...
		if (unlikely(areq_ctx->is_icv_fragmented == true)) {
			...
			if (direct == SEP_CRYPTO_DIRECTION_DECRYPT) {
			  ...
			  dx_buffer_mgr_copy_scatterlist_portion(
				  areq_ctx->backup_mac, req->src,
				  size_to_skip+ req->cryptlen - areq_ctx->req_authsize,
				  size_to_skip+ req->cryptlen, DX_SG_TO_BUF);
		...
		icv_nents = dx_buffer_mgr_get_aead_icv_nents(areq_ctx->dstSgl,
	...
	bool is_last_table, bool do_chain)
{
    ...
    src_mapped_nents = dx_buffer_mgr_get_sgl_nents(req->src,size_for_map,&src_last_bytes, &chained);  
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,3,0)
  dst_mapped_nents = dx_buffer_mgr_get_sgl_nents(req->dst,size_for_map,&dst_last_bytes, &chained);
