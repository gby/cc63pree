...
/* List of DBs */
static struct ssi_stat_item stat_host_db[MAX_STAT_OP_TYPES][MAX_STAT_PHASES];
static struct ssi_stat_item stat_cc_db[MAX_STAT_OP_TYPES][MAX_STAT_PHASES];


static void init_db(struct ssi_stat_item item[MAX_STAT_OP_TYPES][MAX_STAT_PHASES])
{
	unsigned int i, j;
...
static void update_db(struct ssi_stat_item *item, unsigned int result)
{
	item->count++;
	item->sum += result;
	if (result < item->min)
...
static void display_db(struct ssi_stat_item item[MAX_STAT_OP_TYPES][MAX_STAT_PHASES])
{
	unsigned int i, j;
	uint64_t avg;
